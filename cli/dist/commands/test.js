...
export async function test(options) {
    ...
    if (!fs.existsSync(configPath)) {
        ...
        p.log.info("Run `craftcode init --challenge <slug>` first to initialize a challenge.");
        process.exit(1);
    }
    // Load config
    let config;
    ...
    p.log.info(`Lesson ${config.challenge.lessonOrder}: ${config.challenge.lessonTitle}`);
    p.log.info(`Environment: ${pc.yellow(config.environment.type.toUpperCase())}`);
    p.log.info("");
    ...
    const testsResponse = await apiRequest(config.testing.testEndpoint);
    ...
    const results = await runAllTests(tests, binaryPath, config.environment, config.testing.stopOnFirstFailure, (result, index, total) => {
        const testNum = index + 1;
        const testCase = tests[index];
        if (result.passed) {
            p.log.success(`${pc.green("✓")} Test ${testNum}/${total}: ${testCase.name} ${pc.dim(`(${result.duration}ms)`)}`);
        }
        else {
            p.log.error(`${pc.red("✗")} Test ${testNum}/${total}: ${testCase.name} ${pc.dim(`(${result.duration}ms)`)}`);
    ...
    // Show summary
    if (allPassed) {
        p.log.success(pc.green(`✓ All ${passedCount} tests passed!`) +
        ...
        if (firstFailure && firstFailure.error) {
            ...
            p.note(firstFailure.error, `Failure: ${failedTest?.name || "Unknown test"}`);
        }
    }
    ...
    const resultResponse = await apiRequest(config.testing.resultEndpoint, {
        method: "POST",
        body: JSON.stringify({
    ...
    if (resultResponse.ok && resultResponse.data) {
        ...
        p.log.info(`Progress: ${data.progress.completed}/${data.progress.total} lessons (${data.progress.percentage}%)`);
        if (data.lessonComplete) {
            p.log.info("");
            p.log.success(pc.bold(`🎉 Lesson ${config.challenge.lessonOrder} complete!`));
            if (data.challengeComplete) {
                p.log.success(pc.bold("🏆 Challenge Complete! Congratulations!"));
            }
            else if (data.nextLesson) {
                p.log.info("");
                p.log.info(`Next: Lesson ${data.nextLesson.order} - ${data.nextLesson.title}`);
                ...
                if (!p.isCancel(continueNext) && continueNext) {
                    ...
                    const configResponse = await apiRequest(`/api/cli/challenge/${encodeURIComponent(config.challenge.slug)}/config`);
                    if (configResponse.ok && configResponse.data) {
                        ...
                        p.log.info(`Run ${pc.cyan("craftcode init --challenge " + config.challenge.slug)} to update your config.`);
                    }
                }
            }
        }
        ...
        p.log.warning("Tests ran locally but failed to save progress. Try again later.");
    }
    p.log.info("");
    if (allPassed) {
        p.outro("Great work! 🚀");
