...
export async function test(options: { challenge?: string }): Promise<void> {
  ...
  const stageResponse = await apiRequest<StageResponse>(endpoint);
  ...
  if (result.success) {
    ...
    const completeResponse = await apiRequest<CompleteResponse>('/api/cli/complete', {
      method: 'POST',
